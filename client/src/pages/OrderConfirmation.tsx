...
import { confirmPixPayment, getSettings, Settings, getOrderByNumber } from '@/lib/api';
...
export default function OrderConfirmation() {
  ...
  async function loadOrderData() {
    ...
    try {
      ...
      const storedTotal = localStorage.getItem(`order_${orderId}_total`);
      if (storedTotal) {
      ...
      // Nota: precisaria de uma rota para buscar pedido por ID
      // Por enquanto, vamos usar o valor do localStorage
    } catch (error) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="max-w-md w-full">
        ...
        <CardContent className="space-y-6">
          {/* Total Amount */}
          {orderTotal && (
            <div className="bg-[#6BAFB8]/10 border-2 border-[#6BAFB8] rounded-lg p-4">
              <p className="text-sm font-medium text-muted-foreground mb-1">Valor Total a Pagar:</p>
              <p className="text-3xl font-bold text-[#6BAFB8]">
