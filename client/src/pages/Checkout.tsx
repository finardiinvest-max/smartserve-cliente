...
export default function Checkout() {
  ...
  const getSelectedTimeLabel = () => {
    if (!formData.horarioRetirada) return '';
    const selected = timeOptions.find(opt => opt.value === formData.horarioRetirada);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (formData.tipoRetirada === 'depois' && !formData.horarioRetirada) {
    ...
    try {
      ...
      // Salvar valor total no localStorage para tela de pagamento PIX
      if (formData.formaPagamento === 'pix') {
        localStorage.setItem(`order_${response.order._id}_total`, total.toString());
      }
      
      clearCart();
      
      // Redirecionar para a mesma rota de acompanhamento
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {formData.tipoRetirada === 'depois' && (
                    <div className="space-y-3">
                      ...
                      {formData.horarioRetirada && (
                        <div className="bg-[#6BAFB8]/10 border border-[#6BAFB8] rounded-lg p-4">
                          <div className="flex items-center gap-2">
                            <span className="text-2xl">🕐</span>
                            <div>
                              <p className="text-sm font-medium text-muted-foreground">Horário Selecionado:</p>
                              <p className="text-lg font-bold text-[#6BAFB8]">
